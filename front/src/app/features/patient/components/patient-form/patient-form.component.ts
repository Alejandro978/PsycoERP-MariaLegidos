  ...
  private populateForm(): void {
    if (this.patient) {
      this.patientForm.patchValue({
        first_name: (this.patient.first_name || '').trim(),
        last_name: (this.patient.last_name || '').trim(),
        email: (this.patient.email || '').trim(),
        phone: (this.patient.phone || '').toString().replace(/\s+/g, '').trim(),
        dni: (this.patient.dni || '').trim().toUpperCase(),
        birth_date: this.patient.birth_date || '',
        gender: this.patient.gender || '',
        occupation: (this.patient.occupation || '').trim(),
        street: (this.patient.street || '').trim(),
        street_number: (this.patient.street_number || '').trim(),
        door: (this.patient.door || '').trim(),
        postal_code: (this.patient.postal_code || '').trim(),
        city: (this.patient.city || '').trim(),
        province: (this.patient.province || '').trim(),
        ...
        progenitor1_full_name: (this.patient.progenitor1_full_name || '').trim(),
        progenitor1_dni: (this.patient.progenitor1_dni || '').trim().toUpperCase(),
        progenitor1_phone: (this.patient.progenitor1_phone || '').toString().replace(/\s+/g, '').trim(),
        progenitor2_full_name: (this.patient.progenitor2_full_name || '').trim(),
        progenitor2_dni: (this.patient.progenitor2_dni || '').trim().toUpperCase(),
        progenitor2_phone: (this.patient.progenitor2_phone || '').toString().replace(/\s+/g, '').trim(),
      });
